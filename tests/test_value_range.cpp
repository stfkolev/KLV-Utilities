...
template<typename T>
void test_unit_range(const klv::value_range<T>& rgn, const T& a, const T& b) {
    using std::size_t;
    using difference_type = typename klv::value_range<T>::difference_type;
...
void test_stepped_range(const klv::stepped_value_range<T, S>& rgn, const T& a, const T& b, const S& s) {
    using std::size_t;
    using difference_type = typename klv::value_range<T>::difference_type;
...
TEST(ValueRanges, IntRanges) {
    ...
    using irange = klv::value_range<int>;
...
TEST(ValueRanges, SizeRanges) {
    using std::size_t;
    using srange = klv::value_range<std::size_t>;
...
TEST(ValueRanges, DoubleRanges) {
    using drange = klv::value_range<double>;
...
TEST(ValueRanges, CharRanges) {
    using crange = klv::value_range<char>;
...
TEST(ValueRanges, StlAlgorithms) {
    ...
    klv::value_range<int> rgn(3, 8); // 3, ..., 7
...
TEST(SteppedRanges, Basics) {
    using std::size_t;
    using srange = klv::stepped_value_range<std::size_t, std::size_t>;
