...
void test_try_parse(const char *sz, bool expect_ret, T expect_val) {
    ...
    x = static_cast<T>(0);
    ASSERT_EQ(expect_ret, try_parse(klv::string_view(sz), x));
    if (expect_ret) {
        ASSERT_EQ(expect_val, x);
    } else {
        ASSERT_EQ(0, x);
