...
#ifndef KLV_MPARSER__
#define KLV_MPARSER__

#include <klv/stringex.hpp>


#define KLV_REQUIRE_CHART_PRED(P) \
    KLV_REQUIRE(std::is_same<typename std::result_of<P(CharT)>::type, bool>::value)

#define KLV_REQUIRE_CHAR_PRED(P) \
    KLV_REQUIRE(std::is_same<typename std::result_of<P(char)>::type, bool>::value)

#define KLV_REQUIRE_MPAR_RULE(Rule) \
    KLV_REQUIRE(std::is_same<typename std::result_of<Rule(const char*, const char*)>::type, mparser>::value)
...
template<class R,
         KLV_REQUIRE_MPAR_RULE(R)>
inline either_of_t<R> either_of(const R& r) {
    return r;
}

template<class R1, class R2,
         KLV_REQUIRE_MPAR_RULE(R1),
         KLV_REQUIRE_MPAR_RULE(R2)>
